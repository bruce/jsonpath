...
class ReferenceTest < Test::Unit::TestCase
  ...
  context 'Sample 1' do
    ...
    should 'resolve a path with a space' do
      assert_resolves("$.'c d'", ["e"])
    end
    should 'resolve a star' do
      assert_resolves("$.*", ["a", "b", "e"])
    end
  ...
  context 'Sample 2' do
    ...
    should 'resolve another number in brackets' do
      assert_resolves("$[4]", [nil])
    end
    should 'resolve a star in brackets' do
      assert_resolves("$[*]", [1, "2", 3.14, true, nil])
    end
    should 'resolve an end slice' do
      assert_resolves("$[-1:]", [nil])
    end
  end
  context 'Sample 3' do
  ...
  context 'Sample 4' do
    ...
    should 'resolve testing on attribute' do
      assert_resolves("$.menu.items[?(@ && @.id && !@.label)].id", ["Open", "Quality", "Pause", "Mute", "Copy", "Help"])
    end
    should 'resolve testing on attribute with regular expression' do
      assert_resolves("$.menu.items[?(@ && @.label && /SVG/.test(@.label))].id", ["CopySVG", "ViewSVG"])
    end
    should 'resolve on negative' do
      assert_resolves("$.menu.items[?(!@)]", [])
    end
    should 'resolve descendant with number in brackets' do
      assert_resolves("$..[0]", [{"id" => "Open"}])
    end
  end
  context 'Sample 5' do
    setup { @json = %({"a":[1, 2, 3, 4],"b":[5, 6, 7, 8]}) }
    should 'resolve descendant with number in brackets' do
      assert_resolves("$..[0]", [1, 5])
    end
    should 'resolve descendant last items' do
      assert_resolves("$..[-1:]", [4, 8])
    end
    should 'resolve by descendant value' do
      assert_resolves("$..[?(@%2==0)]", [2, 4, 6, 8])
    end
  end
  context 'Sample 6' do
    ...
    should 'resolve by operation in quotes' do
      assert_resolves("$.'?(@.color)'.x", [])
    end
    should 'resolve by multiple quoted values in brackets' do
      assert_resolves("$['lin','cir'].color", ["red", "blue"])
    end
  end
  context 'Sample 7' do
    setup { @json = %({"text":["hello", "world2.0"]}) }
    should 'resolve correctly' do
      assert_resolves("$.text[?(@.length > 5)]", ["world2.0"])
    end
    should 'resolve correctly' do
      assert_resolves("$.text[?(@.charAt(0) == 'h')]", ["hello"])
    end
  end
  context 'Sample 8' do
    setup { @json = %({"a":{"a":2,"b":3},"b":{"a":4,"b":5},"c":{"a":{"a":6,"b":7},"c":8}}) }
    should 'resolve descendant' do
      assert_resolves("$..a", [{"a" => 2, "b" => 3}, 2, 4, {"a" => 6, "b" => 7}, 6])
    end
  end
  context 'Sample 10' do
    ...
    should 'resolve with quoted operation and escaped special character' do
      assert_resolves("$.a[?(@['\\@']==3)]", [{"a" => 6, "@" => 3, "$" => 4}])
    end
    should 'resolve with quotes and brackets in operation' do
      assert_resolves("$.a[?(@['$']==5)]", [{"a" => 7, "@" => 4, "$" => 5}])
    end
  end
  
  private
  
  def assert_resolves(path, result)
    assert_equal result, JSON.path(@json, path).to_a
  end
